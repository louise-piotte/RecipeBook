...
- Ingredient-aware regeneration must not overwrite shared `IngredientLine.originalText` or `ingredientName` when generating the opposite recipe language because those fields are still single-language source data; only bilingual recipe text and ingredient-reference suggestions should change.
